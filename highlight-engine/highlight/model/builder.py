...
def is_highlight_model(model_path, model_name, token=None):
    if "highlight" in model_name.lower():
        return True
    ...
    return hasattr(config, "time_vocab_size") and hasattr(config, "score_vocab_size")
...
def load_pretrained_model(model_path, model_base, model_name, load_8bit=False, load_4bit=False, device_map="auto", device="cuda", use_flash_attn=False, **kwargs):
    ...
    if is_highlight_model(model_path, model_name, token=token):
        # Load LLaVA model
        ...
        if 'lora' in model_name.lower() and model_base is not None:
            ...
            print('Loading highlight model from base model...')
            model = HighlightLlamaForCausalLM.from_pretrained(model_base, low_cpu_mem_usage=True, config=lora_cfg_pretrained, **kwargs)
            token_num, tokem_dim = model.lm_head.out_features, model.lm_head.in_features
            if model.lm_head.weight.shape[0] != token_num:
                model.lm_head.weight = torch.nn.Parameter(torch.empty(token_num, tokem_dim, device=model.device, dtype=model.dtype))
                model.model.embed_tokens.weight = torch.nn.Parameter(torch.empty(token_num, tokem_dim, device=model.device, dtype=model.dtype))

            print('Loading additional Highlight weights...')
            ...
            model = HighlightMistralForCausalLM.from_pretrained(model_base, low_cpu_mem_usage=True, config=cfg_pretrained, **kwargs)
            ...
            model = HighlightMistralForCausalLM.from_pretrained(model_path, low_cpu_mem_usage=True, **kwargs)
    else:
        # Load language model
    ...
    if is_highlight_model(model_path, model_name, token=token):
        ...
        # NOTE: Highlight adopts the same processor for processing image and video.
