...
from highlight import conversation as conversation_lib
from highlight.constants import NUM_FRAMES, IGNORE_INDEX, MMODAL_TOKEN_INDEX, DEFAULT_MMODAL_TOKEN, DEFAULT_MMODAL_START_TOKEN, DEFAULT_MMODAL_END_TOKEN, MMODAL_INDEX_TOKEN
from highlight.highlight_trainer import HighlightTrainer
from highlight.model import *
from highlight.mm_utils import tokenizer_MMODAL_token, tokenizer_image_token, expand2square, process_video, process_image, tokenizer_MMODAL_token_all
...
def train(attn_implementation=None):
    ...
    if model_args.vision_tower is not None:
        if 'vicuna' in model_args.model_name_or_path.lower():
            ...
            model = HighlightLlamaForCausalLM.from_pretrained(
            ...
            config._attn_implementation = attn_implementation
            config.downsample_num = model_args.downsample_num
            model = HighlightMixtralForCausalLM.from_pretrained(
                model_args.model_name_or_path,
                config=config,
                cache_dir=training_args.cache_dir,
                torch_dtype=(torch.bfloat16 if training_args.bf16 else None),
            ...
            config._attn_implementation = attn_implementation
            config.downsample_num = model_args.downsample_num
            model = HighlightQwen2ForCausalLM.from_pretrained(
                model_args.model_name_or_path,
                config=config,
                cache_dir=training_args.cache_dir,
                torch_dtype=(torch.bfloat16 if training_args.bf16 else None),
            ...
            config._attn_implementation = attn_implementation
            config.downsample_num = model_args.downsample_num
            model = HighlightMistralForCausalLM.from_pretrained(
                model_args.model_name_or_path,
                config=config,
                cache_dir=training_args.cache_dir,
                torch_dtype=(torch.bfloat16 if training_args.bf16 else None),
    ...
    # select a Trainer
    trainer = HighlightTrainer(model=model, tokenizer=tokenizer, args=training_args, **data_module)
