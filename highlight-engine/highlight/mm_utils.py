...
def process_video(video_path, processor, aspect_ratio='pad', num_frames=NUM_FRAMES, image_grid=False, sample_scheme='uniform'):
    def frame_sample(duration, mode='uniform', local_fps=None):
        if mode == 'uniform':
            ...
            intervals = np.linspace(0, duration-1, num=num_frames + 1).astype(int)
            ranges = []
            for idx, interv in enumerate(intervals[:-1]):
                ranges.append((interv, intervals[idx + 1] - 1))
            indices = [random.choice(np.linspace(x[0], x[1], num=6).astype(int)[:-1]) for x in ranges]
            # print(indices, ranges)
            # indices = [random.choice(range(x[0], x[1])) for x in ranges]
            return indices
        else:
    ...
    if isinstance(video_path, str):
        if video_path.endswith('.gif'):
            ...
            video_timestamps = [[index / local_fps] for index, frame in enumerate(video_gif) if index in frame_id_list]
        # added by lixin4ever, include the support of .webm files from sthsthv2
            ...
            video_timestamps = [[idx / local_fps] for idx in frame_id_list]
        else:
    ...
    # print(num_frames, len(video_timestamps))

    if video_timestamps[-1][0] > 9999:
        raise ImportError(f'The video is too long!')
    if video_timestamps[0][0] < 0:
        raise ImportError(f'Timestamp can not be less than zero')

    return video, video_timestamps
...
####################################################################################################################################################################
def tokenizer_MMODAL_token_all(prompt, tokenizer, return_tensors=None):

    # print([prompt])

    pattern = '|'.join(map(re.escape, DEFAULT_MMODAL_TOKEN.values()))

    # print(pattern)

    # prompt_chunks = [tokenizer(chunk + '<sync>').input_ids for chunk in re.split(pattern, prompt)]
    # print(re.split(pattern, prompt), prompt_chunks)
    prompt_chunks = [tokenizer(chunk).input_ids for chunk in re.split(pattern, prompt)]
    modal_seps = [MMODAL_TOKEN_INDEX[modal[1:-1].upper()] for modal in re.findall(pattern, prompt)]
    ...
    assert (len(prompt_chunks) == len(modal_seps) + 1) or len(prompt_chunks) == 0

    for x, s in zip(prompt_chunks[1:], modal_seps):
        input_ids.append(s)
        input_ids.extend(x[offset:])
...
class KeywordsStoppingCriteria(StoppingCriteria):
    ...
    def call_for_batch(self, output_ids: torch.LongTensor, scores: torch.FloatTensor, **kwargs) -> bool:
        # print(output_ids, self.keyword_ids, self.keywords)
        generated_ids = output_ids[0]
        generated_ids = generated_ids[generated_ids < self.tokenizer.vocab_size]
        if generated_ids.numel() == 0:
            return False

        generated_ids = generated_ids.unsqueeze(0)
        offset = min(max(generated_ids.shape[1] - self.start_len, 0), self.max_keyword_len)
        keyword_ids_on_device = [keyword_id.to(generated_ids.device) for keyword_id in self.keyword_ids]
        for keyword_id in keyword_ids_on_device:
            if generated_ids.shape[1] >= keyword_id.shape[0] and (generated_ids[0, -keyword_id.shape[0]:] == keyword_id).all():
                return True

        if offset <= 0:
            offset = min(generated_ids.shape[1], self.max_keyword_len)
        outputs = self.tokenizer.batch_decode(generated_ids[:, -offset:], skip_special_tokens=True)[0]
