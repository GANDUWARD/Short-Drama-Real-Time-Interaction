...
OUTPUT_JSON="${OUTPUT_JSON:-${REPO_ROOT}/outputs/download_qvhighlights_cross_env_qwen_framewise_result.json}"
QUERY="${QUERY:-}"
DEVICE="${DEVICE:-cuda}"
DETECT_NUM_FRAMES="${DETECT_NUM_FRAMES:-128}"
DESCRIBE_MAX_NEW_TOKENS="${DESCRIBE_MAX_NEW_TOKENS:-64}"
...
if [[ "${KEEP_TEMP_FILES}" == "true" ]]; then
  STAGE2_ARGS+=(--keep-temp-files)
fi

python3 scripts/inference/run_qwen2_5_vl_caption_frames.py "${STAGE2_ARGS[@]}"
